...
prd <- orders_products %>% #just prior information
  ...
  summarise(
    ...
    prod_avg_reorders = mean(reordered), #average reorder rate for items
...
usaisle <- orders_products %>% left_join(products %>% select(-product_name))%>%
  group_by(user_id, aisle) %>% 
  summarise(n_aisle=n()) %>% mutate(p_aisle=n_aisle/sum(n))
usdep <- orders_products %>% left_join(products %>% select(-product_name))%>%
  group_by(user_id, department) %>% 
  summarise(n_dep=n()) %>% mutate(p_dep=n_dep/sum(n))

#trained on prior information
data <- orders_products %>% left_join(products %>%
  group_by(user_id, product_id) %>% 
  summarise(
    up_orders = n(),
...
submission <- submission %>% bind_rows(missing) %>% arrange(order_id)
write.csv(submission, file = "xgboost_numero1_submission_5.csv", row.names = F)
```
