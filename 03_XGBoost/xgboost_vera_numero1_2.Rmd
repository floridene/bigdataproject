...
* try 600 rounds, min logloss = 0.241410 -> Leaderboard: 0.3813866
* new feature "aisles and departments", all data (train), rounds = 100, min logloss = 0.244480 -> LB: 0.3808102
* model with product_id , all data (train), rounds = 300, min logloss = 0.242679 > Leaderboard:0.3819342
* 1000 rounds (over night)

# Predict on model: 
```{r}
Y <- xgb.DMatrix(as.matrix(test %>% select(-order_id)))#here dont select order and product id as we dont want to predict on them
...
write.csv(submission, file = "xgboost_numero1_submission_8_withProductID.csv", row.names = F)
```
