...
X <- xgb.DMatrix(as.matrix(subtrain %>% select(-reordered, -user_id)), label = subtrain$reordered)
...
Y <- xgb.DMatrix(as.matrix(test %>% select(-user_id)))
...
write.csv(submission, file = "sub_xgb_notscaled_blödsinn_3_alldata.csv", row.names = F)
```
